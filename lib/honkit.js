...
function requireHonkitRuntime() {
  if (!isHonkitRuntimeInstalled()) {
    ...
    console.error('  node <path-to-tutorial-skill-repo>/bin/cli.js docs runtime install');
    console.error('');
    console.error('Then retry:');
    console.error('  node <path-to-tutorial-skill-repo>/bin/cli.js docs preview --dir <path>');
    console.error('');
    console.error('Published-package equivalent (after you publish a release containing docs commands):');
    console.error('  rm -rf ~/.claude/skills/tutorial');
...
function doctorDocs(argv) {
  ...
  console.log('🔎 tutorial-skill docs doctor');
  console.log('');
  ...
  if (hasFailures) {
    ...
    if (
      ...
      console.log('  node <path-to-tutorial-skill-repo>/bin/cli.js docs runtime install');
      console.log('');
      console.log('  # If you installed via npm and your published package is outdated, reinstall after publishing:');
      console.log('  rm -rf ~/.claude/skills/tutorial');
    ...
    if (inspectDocsDir) {
      console.log(`  npx @sshaaf/tutorial-skill docs init --dir ${targetDir}`);
    }
    process.exit(1);
  }
